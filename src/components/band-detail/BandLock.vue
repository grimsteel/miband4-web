  ...
  <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 h-max">
    ...
    <form @submit.prevent="onFormSubmit" class="max-w-md text-gray-900 divide-y divide-gray-200 dark:text-white dark:divide-gray-700">
      <div class="flex flex-col py-2">
        <Toggle v-model="bandLockConfig.enabled" text="Band Lock" />
      </div>
      <div class="flex py-2 items-center justify-between" v-show="bandLockConfig.enabled">
...
<script setup lang="ts">
  ...
  const bandLockConfig = ref(props.bandLock ?? defaultBandLock);

  watch(
    () => props.bandLock,
    bL => bandLockConfig.value = bL ?? defaultBandLock
  );
