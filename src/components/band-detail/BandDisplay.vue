    ...
    <ul class="w-full text-sm font-medium text-gray-900 dark:text-white divide-y divide-gray-200 dark:divide-gray-600 mb-3">
      <li class="w-full flex items-center justify-between" v-for="item, i in displayItems">
        ...
        <div class="inline-flex rounded-md shadow-sm" role="group">
          <button @click="moveItemUp(i)" type="button" class="p-1.5 text-gray-800 border border-gray-200 rounded-l-lg hover:bg-gray-100 hover:text-amber-700 focus:z-10 focus:ring-amber-700 focus:text-blue-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-amber-500 dark:focus:text-white" :class="{ 'rounded-r-lg': i === displayItems.length - 1, 'invisible': i === 0 }">
            <IconChevronUp class="w-4 h-4" />
          </button>
          <button @click="moveItemDown(i)" type="button" class="p-1.5 text-gray-800 border border-gray-200 rounded-r-lg hover:bg-gray-100 hover:text-amber-700 focus:z-10 focus:ring-amber-700 focus:text-blue-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-amber-500 dark:focus:text-white" :class="{ 'rounded-l-lg': i === 0, 'invisible': i === displayItems.length - 1 }">
